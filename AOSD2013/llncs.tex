...
%An event definition is comprised of an event keyword, \lstinline{before} or \lstinline{after}, and a pointcut expression.
...
%\end{figure}
%
%For the declaration \lstinline!instance pointcut subsetIP1 subsetof IP1:...!, the subset relationship imposes that the instance set created by \lstinline{subsetIP1} has to be the subset of the instance set of \lstinline{IP1}. When the \emph{instance type} is not declared like in the example then it is inferred from the referenced instance pointcut, \lstinline{IP1}. It is also possible to declare subsets by using type refinement. For example
%\lstinline!instance pointcut subsetIP1<SubType> subsetof IP1: IP1<SubType>!
%creates a subset of \lstinline{IP1} by selecting the instances of type \lstinline{SubType}. For the subset relationship the referencing type must declare a co-variant type of the referenced instance pointcut.
%For the superset declaration \lstinline!instance pointcut supersetIP1 supersetof IP1:...!, the superset relationship imposes that the instance set created by \lstinline{supersetIP1} contains the set of \lstinline{IP1}. This rule holds even if the scope of \lstinline{supersetIP1} is narrowed. When the instance type is not declared then the instance type of \lstinline{IP1} is copied. For the superset relationship referencing instance pointcut must declare a contra-variant type.
%
...
%
%\begin{lstlisting}[float=h!, caption={The generated AspectJ pointcuts from \lstinline{outOfStock} instance pointcut}, label={lst:generated1}]
%aspect MyAspect{
...
%\end{lstlisting}	
%
%In Listing~\ref{lst:generated2} two advice bodies shown in lines 10--13 and 14--17. The first advice is responsible for performing the add to set action and the second one is responsible for removal. For the \lstinline{returning} clause there is a special case. When it is used in instance pointcut expressions an \lstinline{after() returning(..)} advice is generated to capture the instance. 
%
%\begin{lstlisting}[float=h!, firstnumber = {18},caption={The set add/remove methods for \lstinline{outOfStock} instance pointcut}, label={lst:generated3}]		
...
%
%
%For adding elements to the set, \lstinline{add_outOfStock_set} method (Listing~\ref{lst:generated3}, lines 18--21) and for removing \lstinline{remove_outOfStock_set} (lines 22--25) are generated. The set monitoring pointcuts for these methods are \lstinline{outOfStock_add(Product p)} and \lstinline{outOfStock_remove(Product p)}, respectively. 
%
%
...
%\end{lstlisting}
%
%The \lstinline{calculateDamages} method defined in Listing~\ref{lst:setaccess}, uses the set access property by referencing \lstinline{MyAspect.outOfStock}. This statement is transformed into a reference to the \lstinline{Multiset} of \lstinline{outOfStock} as \lstinline{MyAspect.outOfStock_set} in Listing~\ref{lst:generated4}. The advice shown in Listing~\ref{lst:monitor1} is copied verbatim, since it does not use any of the extended syntax.
%
